...
4. Write testable functions that require _zero UI assumptions._
...
_(Activated only if backend syncing is requested.)_
...
3. Enforce determinism — same data must _always_ output the same settlement.
...
Complex tasks can trigger _multiple_ agents in sequence, but never conflict with each other.
