...
describe('FxRateRepository', () => {
  ...
  it('archives previous rate and inserts new version via setRate', async () => {
    ...
    expect(JSON.parse(mockDb.rows.find((r) => r.id === created.id)?.metadata ?? '{}')).toEqual({
      note: 'user override',
    });
  });
  ...
  it('getAllActiveRates returns non-archived ordered by priority then fetchedAt', async () => {
    ...
    expect(rates.map((r) => r.id)).toEqual(['older-high-priority', 'newer-lower-priority']);
  });
  ...
  it('archives specific rate by id', async () => {
    mockDb.reset([
      createRateRow({ id: 'to-archive', baseCurrency: 'USD', quoteCurrency: 'MXN', isArchived: false }),
    ]);

    await FxRateRepository.archiveRate('to-archive');

    expect(mockDb.rows.find((r) => r.id === 'to-archive')?.isArchived).toBe(true);
  });
});
