...
describe("Settlement Integration", () => {
  ...
  describe("correctness properties", () => {
    it("should always maintain conservation of money", () => {
      ...
      const totalPositive = balances.reduce(
        (sum, b) => sum + Math.max(0, b.netPosition),
        0,
      );
      const totalSettled = settlements.reduce((sum, s) => sum + s.amount, 0);
    ...
    it("should be deterministic across multiple runs", () => {
      ...
      const run1 = {
        ...
        settlements: optimizeSettlements(
          calculateBalances(expenses, splits, participants),
        ),
      };
